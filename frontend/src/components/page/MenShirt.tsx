...
import '../../css/men-shirt.css';
import menShirtHero from '../../assets/aothun-promo-banner.jpg';
...
const MenShirt: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter products that are men's shirts (Áo Nam, Áo Thun, Polo, Sơ mi, etc.)
        // This can be based on category name or product name
        if (selectedCategory === 'all') {
          allProducts = allProducts.filter((product: Product) => {
            const categoryName = product.category_id?.name?.toLowerCase() || '';
            const productName = product.name?.toLowerCase() || '';
            return (
              categoryName.includes('áo nam') ||
              categoryName.includes('ao nam') ||
              categoryName.includes('áo thun') ||
              categoryName.includes('ao thun') ||
              categoryName.includes('polo') ||
              categoryName.includes('sơ mi') ||
              categoryName.includes('so mi') ||
              productName.includes('áo nam') ||
              productName.includes('ao nam') ||
              productName.includes('áo thun') ||
              productName.includes('ao thun')
            );
          });
        }
  ...
  // Category options for men's shirts
  const shirtCategories = [
    ...
    { id: 'ao-ba-lo', name: 'Áo ba lỗ' },
    { id: 'ao-ni-va-len', name: 'Áo Nỉ và Len' },
    { id: 'hoodie', name: 'Hoodie' },
    { id: 'set-do', name: 'Set Đồ' },
  ];

  return (
    <div className="men-shirt-page">
        ...
        <Row className="mb-4 hero-section">
          ...
          <Col md={6} className="hero-text-col">
            <div className="hero-text-content">
              <h1 className="hero-title">ÁO NAM</h1>
              <h2 className="hero-subtitle">Men's Shirts</h2>
            </div>
          </Col>
        </Row>
