...
const ProductDetail: React.FC = () => {
  ...
  try {
    const res = await axios.post(
      "https://project-ma-nguon-mo-3.onrender.com/api/api/cart/add",
      {
 ...
 useEffect(() => {
  ...
  const fetchProduct = async () => {
    try {
      const res = await fetch(`https://project-ma-nguon-mo-3.onrender.com/api/api/products/${id}`);
...
const handleBuyNow = () => {
  ...
  const fetchRelated = async (categoryId: string) => {
    try {
      const res = await fetch(`https://project-ma-nguon-mo-3.onrender.com/api/api/products?category_id=${categoryId}`);
      const data = await res.json();
