...
const PaymentMethods: React.FC<PaymentMethodsProps> = ({ user, onUpdate }) => {
  ...
  const handleDelete = async (methodId: string) => {
    ...
    try {
      ...
      await axios.delete(`https://project-ma-nguon-mo-3.onrender.com/api/users/me/payment-methods/${methodId}`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = tokenManager.getAccessToken();
      await axios.post("https://project-ma-nguon-mo-3.onrender.com/api/users/me/payment-methods", formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
