...
const handleGoogleResponse = useCallback(
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await axios.post("https://project-ma-nguon-mo-3.onrender.com/api/users/login", {
        email: emailOrPhone,
        password: password,
      });
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axios.post("https://project-ma-nguon-mo-3.onrender.com/api/users/register", {
        name,
        phone,
        email,
